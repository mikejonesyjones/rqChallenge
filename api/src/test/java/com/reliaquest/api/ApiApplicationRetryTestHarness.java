...
public class ApiApplicationRetryTestHarness
{
    ...
    public ApiApplicationRetryTestHarness() throws IOException, ExecutionException, InterruptedException
    {
        ...
        CompletableFuture<Process> apiFuture = TestUtils.startServer("api/build/libs/api-1.0.0.jar",
                                                                     apiServerPort,
                                                                     "Started ApiApplication",
                                                                     "--employee.api.port=" + serverPort);
        CompletableFuture<Process> serverFuture = TestUtils.startServer("server/build/libs/server-1.0.0.jar",
                                                                        serverPort,
                ...
                executor.submit(() -> {
                    ...
                    try
                    {
                        ...
                        outcome = response.getStatusCode()
                                          .is2xxSuccessful() ? Outcome.SUCCESS : Outcome.FAILED;
                    }
    ...
    private double failureRate()
    {
        ...
        long failures = results.stream().filter(r -> r.outcome != Outcome.SUCCESS).count();
        return (double) failures / total;
    }

    private void printSummary()
    {
        long success = results.stream().filter(r -> r.outcome == Outcome.SUCCESS).count();
        long rejected = results.stream().filter(r -> r.outcome == Outcome.REJECTED).count();
        long failed = results.stream().filter(r -> r.outcome == Outcome.FAILED).count();
        double avgLatency = results.stream().mapToLong(r -> r.duration).average().orElse(0.0);
